...
!macro CheckAllVergeProcesses
    ; Check if Clash-Verge-Buty.exe is running
    nsis_tauri_utils::FindProcess "Clash-Verge-Buty.exe"
    ${If} $R0 != 0
        ; Kill the process
        DetailPrint "Kill Clash-Verge-Buty.exe..."
        !if "${INSTALLMODE}" == "currentUser"
            nsis_tauri_utils::KillProcessCurrentUser "Clash-Verge-Buty.exe"
        !else
            nsis_tauri_utils::KillProcess "Clash-Verge-Buty.exe"
        !endif
    ${EndIf}
...
!macro StartVergeService
  ...
  SimpleSC::ExistsService "clash_verge_buty_service"
  Pop $0  ; 0：service exists；other: service not exists
  ; Service exists
  ${If} $0 == 0
    Push $0
    ; Check if the service is running
    SimpleSC::ServiceIsRunning "clash_verge_buty_service"
    Pop $0 ; returns an errorcode (<>0) otherwise success (0)
    Pop $1 ; returns 1 (service is running) - returns 0 (service is not running)
    ${If} $0 == 0
      Push $0
      ${If} $1 == 0
            DetailPrint "Restart Clash-Verge-Buty Service..."
            SimpleSC::StartService "clash_verge_buty_service" "" 30
      ${EndIf}
    ${ElseIf} $0 != 0
          Push $0
...
!macro RemoveVergeService
  ...
  ${If} $0 == 0
    ...
    ${If} $0 == 0
      Push $0
      ${If} $1 == 1
        DetailPrint "Stop Clash-Verge-Buty Service..."
        SimpleSC::StopService "clash_verge_buty_service" 1 30
        Pop $0 ; returns an errorcode (<>0) otherwise success (0)
        ${If} $0 == 0
              DetailPrint "Removing Clash-Verge-Buty Service..."
              SimpleSC::RemoveService "clash_verge_buty_service"
        ${ElseIf} $0 != 0
                  Push $0
                  SimpleSC::GetErrorMessage
                  Pop $0
                  MessageBox MB_OK|MB_ICONSTOP "Clash-Verge-Buty Service Stop Error ($0)"
        ${EndIf}
  ${ElseIf} $1 == 0
        DetailPrint "Removing Clash-Verge-Buty Service..."
        SimpleSC::RemoveService "clash_verge_buty_service"
