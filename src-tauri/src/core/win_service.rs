...
pub(super) async fn get_service_health() -> Result<HealthResponse> {
    let res = reqwest::ClientBuilder::new()
        ...
        .json::<HealthResponse>()
        .await
        .context("failed to query clash-verge-service health endpoint")?;
    Ok(res)
}

pub(super) async fn get_service_clash_state() -> Result<JsonResponse> {
    let res = reqwest::ClientBuilder::new()
        .no_proxy()
        .build()?
        ...
        .json::<JsonResponse>()
        .await
        .context("failed to query clash-verge-service clash state endpoint")?;
    Ok(res)
}
