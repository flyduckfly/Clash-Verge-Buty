...
pub struct CoreManager {
    ...
    #[allow(unused)]
    // NOTE: in-memory flag for last start strategy; not actual Windows service runtime state.
...
impl CoreManager {
    ...
    pub async fn run_core(&self) -> Result<()> {
        ...
        let last_started_by_service_mode = *self.use_service_mode.lock();
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            log::info!(
                target: "app",
                "run_core decision input: desired_service_mode={}, last_started_by_service_mode={}, service_running={}, service_core_pid={:?}, sidecar_pid={:?}, pid_file_pid={:?}, runtime_config_path={}",
                desired_service_mode,
                last_started_by_service_mode,
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if self.sidecar.lock().is_none() {
                #[cfg(target_os = "windows")]
                {
                    ...
                    log::info!(target: "app", "recover_core decision input: reason=sidecar_terminated, desired_service_mode={}, last_started_by_service_mode={}, service_running={}, service_core_pid={:?}, sidecar_pid={:?}, pid_file_pid={:?}", desired_service_mode, last_started_by_service_mode, service_running, service_core_pid, sidecar_pid, pid_file_pid);
    ...
    pub fn stop_core(&self) -> Result<()> {
        ...
        tauri::async_runtime::block_on(async move {
            ...
            let service_status = super::win_service::check_service().await.ok();
            let service_core_pid = service_status.as_ref().and_then(|s| s.core_pid);
            log::info!(target: "app", "stop_core decision input: desired_service_mode={}, service_running={}, service_core_pid={:?}", desired_service_mode, service_status.as_ref().map(|s| s.running).unwrap_or(false), service_core_pid);
