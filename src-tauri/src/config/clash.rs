...
impl IClashTemp {
    pub fn new() -> Self {
        ...
        match dirs::clash_path().and_then(|path| help::read_merge_mapping(&path)) {
            Ok(mut map) => {
                let mut has_materialized = false;
                ...
                let guarded = Self::guard(map);
                if has_materialized {
                    if let Ok(path) = dirs::clash_path() {
                        if let Err(err) =
                            help::save_yaml(&path, &guarded, Some("# Generated by Clash-Verge-Buty"))
                        {
    ...
    pub fn template() -> Self {
        ...
        tun.insert("device".into(), "Clash-Verge-Buty".into());
    ...
    pub fn save_config(&self) -> Result<()> {
        help::save_yaml(
            &dirs::clash_path()?,
            &self.0,
            Some("# Generated by Clash-Verge-Buty"),
        )
    }
